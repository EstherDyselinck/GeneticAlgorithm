class DNA {
  ...
  DNA() {
    ...
    for (int yF=0; yF<gridDimension; yF++) {                               // Loop over alle genoParts (de y-richting)
      for (int zF=0; zF<gridHeight; zF++) {                                // Loop over alle intervallen in 1 genoPart (de z-richting)
        for (int xF=0; xF<gridDimension; xF++) {                           // Loop over elke rij in de x-richting
          if (xF<geno.getGenoPart(yF).getInterval(zF).getCoo(0))                                                                genes[xF][yF][zF]=false;
          else if (xF>=geno.getGenoPart(yF).getInterval(zF).getCoo(0) && xF<=geno.getGenoPart(yF).getInterval(zF).getCoo(1))    genes[xF][yF][zF]=true;
          else if (xF>=geno.getGenoPart(yF).getInterval(zF).getCoo(1))                                                          genes[xF][yF][zF]=false;
        }
      }
    }
  }
  ...
  //----------------------------------------------------------------------------------------------------------------------[Get Interval]
  Interval getIntervalPT(int yPT, int zPT) {
    return geno.getIntervalGT(yPT, zPT);
  }
  
  //----------------------------------------------------------------------------------------------------------------------[Set new interval]
  void setNewIntervalPT(int yPT, int zPT, Interval newIntervalPT) {
    geno.setNewIntervalGT(yPT, zPT, newIntervalPT);
    for (int xF=0; xF<gridDimension; xF++) {
      if (xF<geno.getGenoPart(yPT).getInterval(zPT).getCoo(0))                                                                genes[xF][yPT][zPT]=false;
      else if (xF>=geno.getGenoPart(yPT).getInterval(zPT).getCoo(0) && xF<=geno.getGenoPart(yPT).getInterval(zPT).getCoo(1))  genes[xF][yPT][zPT]=true;
      else if (xF>=geno.getGenoPart(yPT).getInterval(zPT).getCoo(1))                                                          genes[xF][yPT][zPT]=false;
    }
  }
